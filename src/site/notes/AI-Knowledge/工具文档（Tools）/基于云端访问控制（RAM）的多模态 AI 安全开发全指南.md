...
这是知识文档的第三部分：**OSS 私有存储桶创建与文件管理**。在这一阶段，我们将为多模态文件（视频、图片、音频）建立一个安全、高效的“家”。
...
既然 Bucket 是私有的，大模型（尤其是国外模型或非同空间的模型）怎么读取它呢？这就需要 SDK 发挥作用了：

1. **你的代码**（持有子账号 AccessKey）会计算出一个包含加密信息的 URL。
    
2. **门票示例**：`https://bucket.oss-cn.hz.aliyuncs.com/photo.jpg?Expires=17000&Signature=xyz...`
